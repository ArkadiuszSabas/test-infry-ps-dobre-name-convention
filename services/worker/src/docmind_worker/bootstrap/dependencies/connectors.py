"""Worker connector manifest dependency factories."""
...
from docmind_worker.application.connectors.service import WorkerConnectorExtensionService
from docmind_worker.settings import ConnectorProfileSettings, load_connector_profile_settings

_CONNECTOR_MANIFEST_STATE_KEY = "worker_connector_profile_manifest"
...
def get_connector_profile_manifest(
    ...
    """Return the app-scoped worker connector profile manifest."""
...
def get_worker_connector_extension_service(
    manifest: Annotated[ProfileManifest, Depends(get_connector_profile_manifest)],
) -> WorkerConnectorExtensionService:
    """Return the worker connector extension service."""

    return WorkerConnectorExtensionService(manifest=manifest)


def load_connector_profile_manifest(settings: ConnectorProfileSettings) -> ProfileManifest:
    """Load the connector profile manifest for worker bootstrap and dependencies."""
